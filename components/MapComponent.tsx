...
// Use CDN URLs for markers to avoid loader/parsing issues with PNG imports
...
// Component to adjust view bounds when route changes
  ...
  useEffect(() => {
    if (route && route.geometry.length > 0) {
      ...
      map.fitBounds(bounds, { padding: [50, 50] });
        ...
        // If no route but we have waypoints, center on the last one, unless user is interacting extensively
        // We keep it simple: pan to latest added.
        const last = waypoints[waypoints.length - 1];
        map.panTo([last.coords.lat, last.coords.lng]);
    }
  }, [route, waypoints.length, map]); 
  // Dependency on waypoints.length instead of waypoints prevents re-centering on every drag
  ...
  return (
    <div className="h-full w-full border-2 border-black grayscale-map relative z-0">
      ...
      >
        ...
        {/* Place Zoom Control in Bottom Right */}
      ...
      {/* Decorative compass rose or similar could go here */}
      <div className="absolute top-4 right-14 md:right-4 z-[400] bg-white border-2 border-black px-2 py-1 font-mono text-xs shadow-[4px_4px_0px_0px_rgba(0,0,0,1)]">
        DATA: OSM
      </div>
    </div>
  );
